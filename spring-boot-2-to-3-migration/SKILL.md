...
python3 <skill-dir>/scripts/scan_repo.py <repo> \
  --format both \
  --output-dir <repo>/.codex/spring-boot-2-to-3-migration
```

Read `<repo>/.codex/spring-boot-2-to-3-migration/scan.md` first for the high-level picture.
Read `<repo>/.codex/spring-boot-2-to-3-migration/todo.md` next for the file-by-file execution list.
...
5. If any stage fails, stop, read `failure-summary.md`, fix the first failed stage, and rerun that stage before moving forward.
...
```bash
# Standard scan
python3 <skill-dir>/scripts/scan_repo.py <repo> --format both --output-dir <repo>/.codex/spring-boot-2-to-3-migration
...
python3 <skill-dir>/scripts/verify_repo.py <repo> --stage startup --startup-command "<repo-specific startup command>" --output-dir <repo>/.codex/spring-boot-2-to-3-migration
```
